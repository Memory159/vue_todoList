...
export default {
    ...
    data() {
        return {
            // 判断这个事情之前是不是一个空字符串，如果是一个空字符串的话，就让todo是一个空数组，如果不是空字符串的话，就按照正常的存储就行了
            // 因为就算是空字符串，JSON.parse 也会把空字符串转换成一个对象，所以永远不会得到后面的空数组
            todos : JSON.parse(localStorage.getItem('save'))[0].things == '' ?  [] : JSON.parse(JSON.parse(localStorage.getItem('save'))[0].things)
            // todos: JSON.parse(JSON.parse(localStorage.getItem('save'))[0].things) || []
            // todos: JSON.parse('') || [2]
        }
