  ...
  <div class="loginWrapper">
    <div class="login">
      ...
      <div class="login_box">
        ...
        <!-- 可以简写为required -->
        <input type="text" v-model.trim="userName" required /><label
          >用户名</label
        >
      </div>
      <div class="login_box">
      ...
      <div class="login_box">
        <input type="password" v-model="pwdConfirm" required="required" /><label
          >确认密码</label
        >
      </div>
      <div>
        <a href="javascript:void(0)" class="left" @click="toLogin">
          返回登录
          <span></span>
        </a>

        <a href="javascript:void(0)" class="right" @click="doRegister">
          确认注册
          <span></span>
        </a>
      </div>
...
export default {
  ...
  methods: {
    ...
    doRegister() {
      ...
      // }
      // this.$store.dispatch('userAdd',userObj)
      //  this.$route.push('/')
  ...
  //   if (this.userName == "" || this.password == "" || this.pwdConfirm == "") {
  //       alert("用户名或者密码为空");
  //     } else {
	// 	if (JSON.parse(localStorage.getItem("save"))[0].userName === this.userName) {
  //         alert("用户名已存在");
  //   }else{
  ...
  //     }
  // }
       if(this.username == '' || this.password == '' || this.pwdagain == ''){
                alert("用户名或密码为空");
            }else{
                if(JSON.parse(localStorage.getItem('save'))!= null && JSON.parse(localStorage.getItem('save'))[0].username === this.username){
                alert("用户名已存在");
                }else{
                    ...
                    alert("密码不相符");
                        ...
                        let save = [
                            {username:this.username,password:this.password,things:[]}
                        ]
                        localStorage.setItem('save',JSON.stringify(save));
                        alert("注册成功，去登录");
                        this.$router.push('/');
...
.loginWrapper {
  ...
  height: 100vh;
  /* 背景渐变色 */
  background: linear-gradient(#302f4b, #5698c3);
}
.login {
  /* 弹性布局 让子元素称为弹性项目 */
  display: flex;
