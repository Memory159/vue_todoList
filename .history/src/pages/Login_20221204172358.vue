  ...
  <div class="loginWrapper">
    <div class="login">
      ...
      <div class="login_box">
        ...
        <!-- 可以简写为required -->
        <input type="text" v-model="userName" required /><label>用户名</label>
      </div>
      <div class="login_box">
      ...
      <div>
        <a href="javascript:void(0)" class="left" @click="toList">
          登录
          <span></span>
          <span></span>
          <span></span>
        ...
        <a href="javascript:void(0)" class="right" @click="toRegister">
          注册
          <span></span>
          <span></span>
          <span></span>
...
export default {
  ...
  methods: {
     getDate() {
        var test = localStorage.getItem("save");
        if (test !== null) {
    ...
    toList() {
      ...
      if (this.userName === "" || this.password === "") {
        ...
        if (JSON.parse(localStorage.getItem("save"))!=null && this.userName !== JSON.parse(localStorage.getItem("save"))[0].userName || this.password !== JSON.parse(localStorage.getItem("save"))[0].password) {
          alert("用户名或密码输入有误");
        } else {
          alert("登录成功");
...
.loginWrapper {
  ...
  height: 100vh;
  /* 背景渐变色 */
  background: linear-gradient(#1f2040, #2e317c);
}
.login {
  /* 弹性布局 让子元素称为弹性项目 */
  display: flex;
